...
template <class ShaderClass> class GpuHost : public Host {
  public:
...
template <class ShaderClass> void GpuHost<ShaderClass>::OnRender(Scene& scene) {
...
template <class ShaderClass> void GpuHost<ShaderClass>::RenderImage() {
...
template <class ShaderClass> void GpuHost<ShaderClass>::Resize(int width, int height) {
...
template <class ShaderClass> auto GpuHost<ShaderClass>::TryResizeTarget(int width, int height) -> Status {
    ...
    if (!_target.create(width, height)) {
        return std::unexpected(Error{ErrorCode::Graphics, "Failed to create GPU render target " +
