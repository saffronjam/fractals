...
// The AVX2 workers write iteration counts four lanes at a time, and the per-worker
// section widths are not 4-aligned, so the final worker can write a few columns past
// the last pixel of the bottom row. Pad the fractal buffer by one SIMD group so those
// tail writes stay inside the allocation (they land in padding and are never read).
static constexpr int SimdWriteSlack = 8;
CpuHost::CpuHost(int simWidth, int simHeight)
    : Host(HostType::Cpu, "CPU", simWidth, simHeight),
      ...
      _fractalArray(new int[SimWidth() * SimHeight() + SimdWriteSlack]) {
...
void CpuHost::OnRender(Scene& scene) {
    ...
    scene.Submit(_vertexArray, sf::RenderStates::Default);
...
void CpuHost::ComputeImage() {
    ...
    const double fractalSectionWidth =
        (SimBox().BottomRight.x - SimBox().TopLeft.x) / static_cast<double>(_workers.size());
    ...
    const auto iterations = this->ComputeIterations();
...
void CpuHost::RenderImage() {
    ...
    for (int y = 0; y < simHeight; y++) {
        for (int x = 0; x < simWidth; x++) {
            ...
            const float offset = static_cast<float>(i) / static_cast<float>(iterations) *
                                 static_cast<float>(PaletteManager::PaletteWidth - 1);
...
void CpuHost::Resize(int width, int height) {
    ...
    for (size_t i = 0; i < _vertexArray.getVertexCount(); i++) {
        _vertexArray[i].position =
            sf::Vector2f(static_cast<float>(std::floor(i % width)), static_cast<float>(std::floor(i / width)));
        _vertexArray[i].color = sf::Color{0, 0, 0, 255};
    }

    delete[] _fractalArray;
    _fractalArray = new int[width * height + SimdWriteSlack];
