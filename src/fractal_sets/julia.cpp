...
Julia::Julia(const sf::Vector2f& renderSize) : FractalSet("Julia", FractalSetType::Julia, renderSize) {
...
void Julia::OnUpdate(Scene& scene) {
    ...
    case JuliaState::FollowCursor: {
        if (scene.ViewportPane().Hovered() && !Keyboard::IsDown(sf::Keyboard::Key::LControl)) {
            const auto mousePos = scene.Camera().ScreenToWorld(scene.ViewportPane().MousePosition(false));
...
void Julia::OnRender(Scene& scene) {
    ...
    if (_drawFlags & JuliaDrawFlags_Dot) {
        ...
        scene.Submit(circle, sf::RenderStates::Default);
    }
    ...
    while (Alive) {
        ...
        for (y = ImageTL.y; y < ImageBR.y; y++) {
            ...
            repeat:
                ...
                _mm256_store_si256(reinterpret_cast<__m256i*>(lanes), _n);
                FractalArray[y_offset + x + 0] = static_cast<int>(lanes[3]);
                FractalArray[y_offset + x + 1] = static_cast<int>(lanes[2]);
                FractalArray[y_offset + x + 2] = static_cast<int>(lanes[1]);
                FractalArray[y_offset + x + 3] = static_cast<int>(lanes[0]);
#endif
