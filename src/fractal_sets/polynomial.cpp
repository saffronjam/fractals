...
Polynomial::Polynomial(const sf::Vector2f& renderSize)
    : FractalSet("Polynomial", FractalSetType::Polynomial, renderSize), _constants({1, 0, -2, 2}),
      _desiredConstants(_constants), _startConstants(_constants), _exponents({3, 2, 1, 0}) {
...
void Polynomial::OnUpdate(Scene& scene) {
    ...
    if (_cTransitionTimer <= _cTransitionDuration) {
        ...
        for (int i = 0; i < PolynomialDegree; i++) {
            _constants[i] =
                _startConstants[i] + static_cast<double>(delta) * (_desiredConstants[i] - _startConstants[i]);
        }
