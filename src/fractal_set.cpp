...
SimBox::SimBox(const Position& topLeft, const Position& bottomRight) : TopLeft(topLeft), BottomRight(bottomRight) {}

auto SimBox::IsEqualTo(const SimBox& other) const -> bool {
    return TopLeft == other.TopLeft && BottomRight == other.BottomRight;
}

auto SimBox::IsDifferentFrom(const SimBox& other) const -> bool {
    return !IsEqualTo(other);
}

FractalSet::FractalSet(std::string name, FractalSetType type, const sf::Vector2f& renderSize)
    : _name(std::move(name)), _type(type), _simWidth(renderSize.x), _simHeight(renderSize.y),
      _simBox(Position(), Position()), _axisVA(sf::PrimitiveType::Lines) {
    PaletteManager::Instance().PaletteUpdated.Subscribe([this] {
...
void FractalSet::OnRender(Scene& scene) {
    ...
    if (_drawAxis) {
        scene.Submit(_axisVA, sf::RenderStates::Default);
    }
}
...
auto FractalSet::Hosts() const -> const std::unordered_map<HostType, std::unique_ptr<Host>>& {
    return _hosts;
}

auto FractalSet::ActiveHostType() const -> HostType {
    return _activeHost;
}

void FractalSet::SetHostType(HostType computeHost) {
    if (!_hosts.contains(computeHost)) {
        Log::CoreError("Cannot set missing fractal host");
        return;
    }
...
void FractalSet::SetComputeIterationCount(uint64_t iterations) noexcept {
...
auto FractalSet::ActiveHost() -> Host& {
    if (!_hosts.contains(_activeHost)) {
        Log::CoreError("No active fractal host");
        return *_hosts.begin()->second;
    }
