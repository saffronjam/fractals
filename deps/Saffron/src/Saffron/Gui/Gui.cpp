...
bool
Gui::Property(const String &name, const String &text, const String &buttonName, const Function<void()> &onButtonPress)
{
    ImGui::Text(name.c_str());
    ImGui::NextColumn();
...
bool Gui::Property(const String &name, sf::Vector2f &value, float min, float max, float step, PropertyFlag flags)
{
    ...
    bool changed;
    if (flags == PropertyFlag::Slider)
        changed = ImGui::SliderFloat2(id.c_str(), reinterpret_cast<float *>(&value), min, max);
    else
        changed = ImGui::DragFloat2(id.c_str(), reinterpret_cast<float *>(&value), step, min, max);
...
bool Gui::Property(const String &name, sf::Vector3f &value, float min, float max, float step, PropertyFlag flags,
                   Optional<Function<void()>> fn)
{
    ...
    if (static_cast<int>(flags) & static_cast<int>(PropertyFlag::Color))
        ...
        changed = ImGui::SliderFloat3(id.c_str(), reinterpret_cast<float *>(&value), min, max);
    else
        changed = ImGui::DragFloat3(id.c_str(), reinterpret_cast<float *>(&value), step, min, max);
...
bool Gui::Property(const String &name, sf::Vector4f &value, float min, float max, float step, PropertyFlag flags)
{
    ...
    if (static_cast<int>(flags) & static_cast<int>(PropertyFlag::Color))
        ...
        changed = ImGui::SliderFloat4(id.c_str(), reinterpret_cast<float *>(&value), min, max);
    else
        changed = ImGui::DragFloat4(id.c_str(), reinterpret_cast<float *>(&value), step, min, max);
...
bool Gui::ImageButton(const sf::Texture &texture, const sf::Vector2f &size, int framePadding, const sf::Color &bgColor,
                      const sf::Color &tintColor)
{
    return GuiImpl::ImageButton(texture, size, framePadding, bgColor, tintColor);
}

bool Gui::ImageButton(const sf::Sprite &sprite, int framePadding, const sf::Color &bgColor, const sf::Color &tintColor)
{
    return GuiImpl::ImageButton(sprite, framePadding, bgColor, tintColor);
}

bool Gui::ImageButton(const sf::Sprite &sprite, const sf::Vector2f &size, int framePadding, const sf::Color &bgColor,
                      const sf::Color &tintColor)
{
